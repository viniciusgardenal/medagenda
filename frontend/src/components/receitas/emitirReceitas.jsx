...
import { getPacientes, getMedicamentos, lerReceitas, criarReceita, downloadReceita } from "../../config/apiServices";
...
const EmitirReceitas = () => {
  ...
  const processedReceitas = useMemo(() => {
    ...
    if(filtroMedicamento) {
        items = items.filter(receita => 
            receita.medicamentos.some(medicamento => 
                medicamento.nomeMedicamento.toLowerCase().includes(filtroMedicamento.toLowerCase())
            )
        );
    }
    ...
    items.sort((a, b) => {
        ...
        if (sortField === 'paciente') {
            valA = `${a.paciente?.nome ?? ''} ${a.paciente?.sobrenome ?? ''}`.trim().toLowerCase();
            valB = `${b.paciente?.nome ?? ''} ${b.paciente?.sobrenome ?? ''}`.trim().toLowerCase();
        } else {
            valA = a[sortField];
            valB = b[sortField];
        }
  ...
  const currentReceitas = processedReceitas.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <section className="max-w-6xl mx-auto bg-white rounded-2xl shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg flex items-center justify-between">
            <h3 className="text-sm font-semibold text-blue-800">Total de Receitas</h3>
            <p className="text-lg font-bold text-blue-600 bg-white px-2.5 py-0.5 rounded-full">{receitas.length}</p>
          </div>
          <div className="bg-green-50 border border-green-200 p-3 rounded-lg flex items-center justify-between">
            ...
            <p className="text-lg font-bold text-green-600 bg-white px-2.5 py-0.5 rounded-full">{pacientes.length}</p>
          </div>
          <div className="bg-yellow-50 border border-yellow-200 p-3 rounded-lg flex items-center justify-between">
            <h3 className="text-sm font-semibold text-yellow-800">Medicamentos</h3>
            <p className="text-lg font-bold text-yellow-600 bg-white px-2.5 py-0.5 rounded-full">{medicamentos.length}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-6 p-4 bg-gray-50 rounded-lg border">
            <div className="flex-1">
                ...
                <input
                    ...
                    onChange={(e) => { setFiltroPaciente(e.target.value); setCurrentPage(1); }}
                    className="w-full px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                    placeholder="Digite o nome do paciente..."
                />
            </div>
            <div className="flex-1">
                <label htmlFor="filtroMedicamento" className="block text-sm font-semibold text-gray-700 mb-1">
                    <FaFilter className="inline-block mr-2 h-3 w-3" />
                ...
                <input
                    ...
                    onChange={(e) => { setFiltroMedicamento(e.target.value); setCurrentPage(1); }}
                    className="w-full px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
        ...
        <div className="mt-6 overflow-x-auto rounded-lg shadow-md">
            {isLoading ? (
                <p className="text-center text-gray-500 py-8">Carregando receitas...</p>
            ) : (
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-blue-600 text-white">
                <tr>
                    <TableHeader label="Data" field="createdAt" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                    <TableHeader label="Paciente" field="paciente" sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                    <th className="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider">Medicamentos</th>
                ...
                {currentReceitas.length === 0 ? (
                    <tr>
                        <td colSpan="4" className="px-6 py-4 text-center text-gray-500 text-sm">
                            {receitas.length > 0 ? "Nenhum resultado para o filtro aplicado." : "Nenhuma receita emitida."}
                        </td>
                    </tr>
                ) : (
                    currentReceitas.map((receita) => (
                    <tr key={receita.batchId} className="hover:bg-blue-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{new Date(receita.createdAt).toLocaleDateString("pt-BR")}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{receita.paciente?.nome} {receita.paciente?.sobrenome}</td>
                        ...
                        <div className="flex items-center justify-center gap-4">
                            <button onClick={() => handleViewReceita(receita)} className="text-blue-600 hover:text-blue-800" title="Visualizar Detalhes"><ViewIcon /></button>
                            <button onClick={() => handleDownloadReceita(receita.batchId)} className="text-green-600 hover:text-green-800" title="Baixar Receita em PDF"><DownloadIcon /></button>
                        </div>
                        </td>
                    </tr>
                    ))
        ...
        {isReceitaModalOpen && ( <ReceitaModal isOpen={isReceitaModalOpen} onClose={() => setIsReceitaModalOpen(false)} pacientes={pacientes} medicamentos={medicamentos} onAddReceita={handleAddReceita} /> )}
        {isViewModalOpen && ( <ViewReceitas isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} receita={selectedReceita} /> )}
      </section>
    </div>
  );
};
