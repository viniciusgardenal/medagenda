...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    // {
    //   title: <strong>FUNÇÕES FUNDAMENTAIS</strong>,
    //   roles: ["Diretor", "Atendente", "Médico"],
    // },
  ...
  return (
    <div className="relative min-h-screen">
      ...
      >
            ...
            {filteredMenuItems.map((item, index) => (
              <li key={index}>
                <a
                  ...
                  `}
                >
                  <span className={`ml-3 ${!isOpen && "hidden lg:inline"}`}>
                    {item.title}
                  </span>
                </a>
              </li>
        ...
        {/* Componente de Perfil do Usuário no Rodapé, organizado em linhas */}
        <div
          className={`absolute justify-center items-center bottom-0 m-auto p-3 flex ${
            !isOpen && "lg:p-2"
          }`}
        >
          <div
            className={`flex flex-col items-center justify-center text-center  text-sky-950 rounded-md p-2 ${
              !isOpen ? "justify-center" : "space-x-10"
            }`}
          >
            {/* Botão de Logout visível quando 'isOpen' for verdadeiro */}
            {isOpen && (
              <button
                className="flex items-center text-red-600 font-medium hover:text-red-800 transition duration-150 space-x-2"
                onClick={logout} // Função de logout a ser definida
              >
                <span>Sair</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
