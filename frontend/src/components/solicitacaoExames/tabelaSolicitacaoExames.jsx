  ...
  if (tse.length === 0) {
    return (
      <div className="text-center py-8 bg-gray-50 rounded-md">
        <p className="text-gray-500">Nenhuma solicitação de exame encontrada.</p>
      </div>
    );
  }
            ...
            <tr key={se.idSolicitacaoExame} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap">
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                  ${se.status === 'Concluído' ? 'bg-green-100 text-green-800' : 
                  se.status === 'Pendente' ? 'bg-yellow-100 text-yellow-800' : 
                  se.status === 'Cancelado' ? 'bg-red-100 text-red-800' : 
